...
void LUMethod(tpMatrix *matrix){
  ...
  double l_sum = 0.0, 
        u_sum = 0.0;
#pragma omp parallel shared(A, LU, a, u_sum, l_sum) private(i, j, k) firstprivate(c, r, u, l) num_threads(2)
{
  l = matrix->A[0];

  //caso especial 1a linha e 1a coluna
  #pragma omp for
  for (i = 0; i < c; i++) {              //1a linha e 1a coluna
    LU[i] = A[i];
    if (i > 0){
      LU[i * c] = A[i * c] / l;
    }//end-if (i > 0){
    ...
    for (int k = j + 1; k < r; k++){          //k
      l = 0.0;
      #pragma omp for
      for (int i = 0; i < j; i++){            //i
        l += LU[k * c + i] * LU[i * c + j];
      } //end-for (int i = 0; i < j; i++)
      #pragma omp critical
      {
      l_sum += l;
      }
...
void PrintX(double *X, const int size){
  int mn = size * size;
  FILE *ptr = fopen("solucao_paralela.bin", "w+");
  assert(ptr != NULL);
