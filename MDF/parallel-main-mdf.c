...
void mdf_heat(double ***  __restrict__ u0,
    ...
    int i, j, k, left, right, up, down, top, bottom, center;

    #pragma omp parallel shared(u0, u1) firstprivate(npX, npY, npZ, ix, jy, kz, deltaH, deltaT, tsteps, boundaries) num_threads(2)
    {
      ...
      for ( unsigned int steps = 0; steps < tsteps; steps++){
                ...
                #pragma omp for private(i, j, k, left, right, up, down, top, bottom, center)
                for (unsigned int i = 0; i < npZ; i++){
                  for (unsigned int j = 0; j < npY; j++){
...
void save2bin(double ***u,
             ...
             const unsigned int npZ){
  char fileName[128];
  sprintf(fileName, "%s-%d-%d-%d-log-parallel.bin", __FILE__,  npX, npY, npZ);
